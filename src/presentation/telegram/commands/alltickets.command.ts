 ...
 */
export function createAllTicketsCommand(ticketService: TicketService) {
  return async (ctx: BotContext): Promise<void> => {
    ...
    try {
      ...
      // Получаем все тикеты
      const openTickets = await ticketService.getTicketsByStatus(TicketStatus.OPEN);
      const inProgressTickets = await ticketService.getTicketsByStatus(TicketStatus.IN_PROGRESS);
      const closedTickets = await ticketService.getTicketsByStatus(TicketStatus.CLOSE);
      ...
      if (openTickets.length > 0) {
        ...
        openTickets.forEach((ticket: Ticket) => {
      ...
      if (inProgressTickets.length > 0) {
        ...
        inProgressTickets.forEach((ticket: Ticket) => {
          message += `   #${ticket.getId()} - ${ticket.title}\n`;
      ...
      if (closedTickets.length > 0) {
        ...
        recentClosed.forEach((ticket: Ticket) => {
          message += `   #${ticket.getId()} - ${ticket.title}\n`;
        });
        message += `\n`;
      }
