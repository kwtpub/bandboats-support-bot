 ...
 */
export function createNewTicketCommand(ticketService: TicketService, userService?: UserService) {
 ...
 */
export function createTicketMessageHandler(
  ...
  return async (ctx: BotContext): Promise<void> => {
    ...
    try {
      ...
      if (ctx.session.replyingToTicketId) {
        ...
        if (text.length > 2000) {
          await ctx.reply(
            '⚠️ Сообщение слишком длинное. Максимум 2000 символов.\nПопробуйте снова:',
          );
          return;
        }
            ...
            try {
              ...
              if (assignee) {
                const keyboard = Markup.inlineKeyboard([
                  ...
                  [Markup.button.callback('👁️ Посмотреть', `admin_view_ticket_${ticketId}`)],
                ]);
      ...
      const backButton = Markup.inlineKeyboard([
        [Markup.button.callback('◀️ Назад', 'ticket_back_to_title')],
      ]);
      ...
      if (ctx.session.ticketStep === 'description' && ctx.session.ticketTitle) {
        ...
        const menuButton = Markup.inlineKeyboard([
          [Markup.button.callback('🏠 Вернуться в меню', 'ticket_back_to_menu')],
        ]);
        ...
        await ctx.reply(
          ...
          { parse_mode: 'Markdown', ...menuButton },
        );
      }
    } catch (error) {
      const message = errorHandler.handle(error as Error, {
